...
interface SafetyPrediction {
  ...
  prediction_factors: {
    ...
    similar_demographics_avg: number
  }
...
serve(async (req: Request) => {
  ...
  try {
      ...
      if (matchingScore) {
        return new Response(
          JSON.stringify({
            ...
            confidence: 1.0, // High confidence - we have actual data
    ...
    const demographicMatches = placeTypeScores?.filter(loc => {
      return loc.safety_scores.some((score: any) => 
    ...
    const dataPoints = (placeTypeScores?.length || 0) + (nearbyLocations?.length || 0)
    const confidence = Math.min(dataPoints / 10, 0.8) // Max 80% confidence for predictions
...
function findBestDemographicMatch(scores: any[], demographics: any) {
  // First try exact demographic matches
  for (const score of scores) {
    if (score.demographic_type === 'race_ethnicity' && 
        demographics.race_ethnicity?.includes(score.demographic_value)) {
      return score
    }
    if (score.demographic_type === 'gender' && 
        score.demographic_value === demographics.gender) {
      return score
    }
    if (score.demographic_type === 'religion' && 
        score.demographic_value === demographics.religion) {
      return score
    }
  }
...
function matchesDemographic(score: any, demographics: any): boolean {
